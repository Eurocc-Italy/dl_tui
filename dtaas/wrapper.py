"""
Wrapper which needs to be run providing a --query flag from command line with the path to a file containing an SQL query 
and (optionally) a --script flag with the path to a Python script. This Python script should contain a `main` function
which takes as input a list (of file paths), does some user-defined analysis, and returns a list (of file paths) which
the wrapper then takes and stores in a compressed archive. TODO: S3 implementation and POST to datalake.

Called from command line and given the query and scripts, the wrapper does the following:
